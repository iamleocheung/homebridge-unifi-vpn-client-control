...
module.exports = class UniFiAPI {
  ...
  async getNetworkconfs(site) {
    let response = await this.request('GET', this._prefixUrl(`api/s/${site}/rest/networkconf/`));
    return response.data;
  }

  async getNetworkconf(site, networkconfId, data) {
    let response = await this.request('GET', this._prefixUrl(`api/s/${site}/rest/networkconf/${networkconfId}`), data);
    return response.data;
  }

  async setNetworkconf(site, networkconfId, data) {
    let response = await this.request('PUT', this._prefixUrl(`api/s/${site}/rest/networkconf/${networkconfId}`), data);
    return response.data;
  }
};
